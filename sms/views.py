...
class SmsView(mixins.CreateModelMixin ,
    ...
    def post(self , request , format=None , *args , **kwargs):
        ...
        if smsUser.filter(status=1):
            return CustomJSONRenderer().render({'phone': 'A user is already this phone'***REMOVED*** , status=400)
    ...
    def put(self , request , phone=None , format=None , *args , **kwargs):
        ...
        if smsUser:
            ...
            if smsUser.code_verify == code_verify:
                ...
                return CustomJSONRenderer().render({'message': 'code not math'***REMOVED*** , status=400)
        else:
...
class ReSendSmsView(APIView):
    ...
    def get(self , request , phone='' , format=None , *args , **kwargs):
        ...
        now = datetime.now()
        if smsUser.active_at and smsUser.status == 1:
            return CustomJSONRenderer().render({'message': 'user is old active'***REMOVED*** , status=400)
        ...
        if now.replace(tzinfo=None) > time_app.replace(tzinfo=None):
            ...
            return CustomJSONRenderer().render({'message': 'last time send code tl minutes 5 '***REMOVED*** , status=400)
